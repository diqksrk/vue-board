    ...
    export default {
        ...
        methods: {
            check_input : function() {
                ...
                // params.append('userPw', this.user_pw)

                this.$axios.post('api/server/new_user', {userName : this.user_name, userId : this.user_id, userPw : this.user_pw})
                    .then((response) => {
                    if (response.data == 'success') {
                        alert("가입이 완료되었습니다.")
                        this.$router.push('/board_login')
                    }
                })                
            },
            check_user_id_exist : function(){
                ...
                this.$axios.post('api/server/check_user', {userName : this.user_name, userId : this.user_id, userPw : this.user_pw})
                    .then((response) => {
                        if (response.data.total == 0 && this.user_id.length != 0) {
